...
/**
  ...
  @Description
    This defines the object that manages the sending and receiving of i2c master transactions.
  */
typedef struct
{
    /* Read/Write Queue */
...
#define SLAVE_I2C_GENERIC_DEVICE_TIMEOUT 255
#define SLAVE_I2C_GENERIC_SLAVE_TIMEOUT 255
#define SLAVE_I2C_GENERIC_RETRY_MAX           255
...
void BMI160_Write(uint16_t slaveDeviceAddress,uint16_t dataAddress,uint8_t *pData,uint16_t nCount);
...
void I2C1_Initialize(void)
{
    ...
    // Baud Rate Generator Value: I2CBRG 48;   
    I2C1BRG = 0x30;
    ...
    uint8_t buffer[3]={3,3,3};

    buffer[0]=0X11;
    BMI160_Write(BMI160_ADDRESS,0x7E,buffer,1);
    delay_10ms(1);
 ...
 /*
    buffer[0]=0x40;
    BMI160_Write(BMI160_ADDRESS,0x69,buffer,1);
    delay_10ms(1);
    ...
    BMI160_Write(BMI160_ADDRESS,0x43,buffer,1);
    delay_10ms(1);
 
    buffer[0]=2;
    BMI160_Write(BMI160_ADDRESS,0x41,buffer,1);
    delay_10ms(1);   
  */
...
void BMI160_Write(uint16_t slaveDeviceAddress,uint16_t dataAddress,uint8_t *pData,uint16_t nCount)
{
    ...
    {
        writeBuffer[0] = (uint8_t)(dataAddress);                // high address
        writeBuffer[1] = *pD++;            // low low address
//        writeBuffer[2] = 0;
        timeOut = 0;
        slaveTimeOut = 0;
        while(status != I2C1_MESSAGE_FAIL)
        {
            ...
            {
                if (slaveTimeOut == 65535)  break;
                else    slaveTimeOut++;
            } 
            if ((slaveTimeOut == 65535) || (status == I2C1_MESSAGE_COMPLETE))   break;
            if (timeOut == 10)    break;
            else    timeOut++;
        }
...
void BMI160_Read(uint16_t slaveDeviceAddress,uint16_t address,uint8_t *pD,uint16_t nCount)
{
    ...
    for (counter = 0; counter < nCount; counter++)
    {
        writeBuffer[0] = (uint8_t)(address);                // high address
//        writeBuffer[1] = (uint8_t)(address);            // low low address
        retryTimeOut = 0;
        slaveTimeOut = 0;
        while(status != I2C1_MESSAGE_FAIL)
        {
            I2C1_MasterWrite(writeBuffer,1,slaveDeviceAddress,&status);
            while(status == I2C1_MESSAGE_PENDING)
            {
                if (slaveTimeOut == 65535) return;
                else    slaveTimeOut++;
            }
            if (status == I2C1_MESSAGE_COMPLETE) break;
            if (retryTimeOut == 65535) break;
            else retryTimeOut++;
        }
        if (status == I2C1_MESSAGE_COMPLETE)
        {
            ...
            while(status != I2C1_MESSAGE_FAIL)
            {
                I2C1_MasterRead(pD,1,slaveDeviceAddress,&status);
                while(status == I2C1_MESSAGE_PENDING)
                {
                    if (slaveTimeOut == 65535) return;
                    else slaveTimeOut++;
